...
import 'package:eoffice/model/suratkeluar/MemoKeluarModel.dart';
import 'package:eoffice/util/separator.dart';
import 'package:eoffice/activity/suratkeluar/memokeluar/DetailMemoKeluar.dart';
import 'package:intl/intl.dart';
import 'package:flutter/cupertino.dart';

class MemoKeluarRoute extends CupertinoPageRoute {
  MemoKeluarRoute()
      : super(builder: (BuildContext context) => new MemoKeluar());
  ...
  Widget buildPage(BuildContext context, Animation<double> animation,
    ...
    return new FadeTransition(opacity: animation, child: new MemoKeluar());
  }
}

class MemoKeluar extends StatefulWidget {
  static const String routeName = "/MemoKeluar";

  @override
  MemoKeluarState createState() => MemoKeluarState();

}

class MemoKeluarState extends State<MemoKeluar> with WidgetsBindingObserver {
  ...
  static List<MemoKeluarList> items = new List();
  static List<MemoKeluarList> itemss = new List();
  ...
  Future<MemoKeluarModel> makeRequest() async {

    sharedPreferences = await SharedPreferences.getInstance();
    loginToken = sharedPreferences.getString("loginToken");

    final response =
    await http.get(URL_DOMAIN + '/api/memo?page=' + refreshList.toString() + '&q=' + query + '&token=' +loginToken);

    if (response.statusCode == 200) {
      itemss.clear();
      itemss.addAll(MemoKeluarModel.fromJson(jsonDecode(response.body)).memoKeluar);
      items.clear();
      items.addAll(itemss);
      ...
      return MemoKeluarModel.fromJson(jsonDecode(response.body));
    } else {
  ...
  void filterSearchResults(String query) {
    List<MemoKeluarList> dummySearchList = new List();
    dummySearchList.clear();
    dummySearchList.addAll(itemss);
    if(query.isNotEmpty) {
      List<MemoKeluarList> dummyListData = new List();
      dummyListData.clear();
      dummySearchList.forEach((item) {
        if(item.no_surat.contains(query)) {
  ...
  Widget build(BuildContext context) {
    ...
    final makeBody = Container(
        child: new FutureBuilder<MemoKeluarModel>(
          future: _future,
                                    ...
                                    child: new ListTile(
                                        ...
                                        leading: Container(
                                            ...
                                            child: new InkWell(
                                              ...
                                              onTap: () {
                                                Navigator.push(context, new MaterialPageRoute(builder: (BuildContext context) => new DetailMemoKeluar('${items[i].id}')));
                                              },
                                            )
                                        ),
                                        title:
      ...
      appBar: new AppBar(
          ...
          title: Row(
            children: [
              ...
              Container(
                  padding: const EdgeInsets.all(8.0), child: Text('Memo Keluar', style: TextStyle(fontFamily: 'Source Code Pro', fontSize: 16.0, color: Colors.red)))
            ],
